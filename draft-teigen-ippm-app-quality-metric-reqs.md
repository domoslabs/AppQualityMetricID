...
# Introduction
The goal of this document is to describe the features a network performance framework must have to be understandable to end-users, useful for application developers, and actionable for network operators. The motivaton for choosing users, application developers and operators all at once is this: solutions exist for many of the problems causing high and unstable latency in the Internet, but the incentives to deploy them are relatively weak. Creating a unifying framework for network quality can help make the incentives better aligned. Key properties for achieving better incentives are visibility to user and apps, as well as creating actionable insight for operators. Users and applications need a way to more easily notice when their network connection is causing issues, and those issues must also be made visible to the network operators. We discuss the needs of each of these stakeholders in turn, and describe what it takes for a network quality framework to meet these needs.
